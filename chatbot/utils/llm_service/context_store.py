# chatbot/utils/llm_service/context_store.py
from utils.llm_service.protocol import new_msg_id
import utils.config

# Stores the context for an LLM.
# The context is a list of messages, each with a role and content.
...
class ContextStore:
    def __init__(self, initial_context=None):
        ...
        if initial_context:
            if utils.config.verbose:
                utils.config.debug_print(f"Initializing context store with initial context: {initial_context}")
    ...
    def _prepare_messages(self, messages):
        ...
        for m in messages:
            if utils.config.verbose:
                utils.config.debug_print(f"Preparing message: {m}")
            m = m.copy()
            m["msg_id"] = new_msg_id()
            m.setdefault("meta", {})
            out.append(m)
